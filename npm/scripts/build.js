...
const PLATFORMS = [
  { pkg: 'bua.js-darwin-x64',   archive: `bua-v${version}-x86_64-apple-darwin.tar.gz`,     bin: 'bua'     },
  { pkg: 'bua.js-darwin-arm64', archive: `bua-v${version}-aarch64-apple-darwin.tar.gz`,    bin: 'bua'     },
  { pkg: 'bua.js-linux-x64',    archive: `bua-v${version}-x86_64-linux-musl.tar.gz`,       bin: 'bua'     },
  { pkg: 'bua.js-linux-arm64',  archive: `bua-v${version}-aarch64-linux-musl.tar.gz`,      bin: 'bua'     },
  { pkg: 'bua.js-win32-x64',    archive: `bua-v${version}-x86_64-windows-msvc.zip`,        bin: 'bua.exe' },
];
...
for (const { pkg, archive, bin } of PLATFORMS) {
  ...
  const tmpDir = path.join(platformPkgsDir, `_tmp_${pkg}`);
  ...
  const [, , osName, archName] = pkg.split('-'); // bua.js-{os}-{arch}
