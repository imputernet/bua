...
const PACKAGE_NAME = '@imputer/bua';
...
function main() {
    ...
    if (!srcBin) {
        ...
        const stub = process.platform === 'win32'
            ? `@echo off\necho Bua: binary not found. Please reinstall @imputer/bua or build from source.\nexit /b 1\n`
            : `#!/bin/sh\necho "Bua: binary not found. Please reinstall @imputer/bua or build from source." >&2\nexit 1\n`;
        fs.writeFileSync(destBin, stub);
        if (process.platform !== 'win32') fs.chmodSync(destBin, 0o755);
        console.warn(`[@imputer/bua] Warning: could not find binary for ${pkgName}`);
        return;
    }
    ...
    console.log(`[@imputer/bua] Installed ${pkgName} → ${destBin}`);
}

try {
    main();
} catch (err) {
    console.error(`[@imputer/bua] Install failed: ${err.message}`);
